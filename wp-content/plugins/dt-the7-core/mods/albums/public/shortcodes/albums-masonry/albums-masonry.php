...
require_once  'class-dt-albums-shortcode-html.php';

if ( ! class_exists( 'DT_Shortcode_AlbumsMasonry', false ) ):

	class DT_Shortcode_AlbumsMasonry extends DT_Shortcode_With_Inline_Css {
		...
		/**
		 * @var DT_Shortcode_AlbumsMasonry
		 */
		public static $instance = null;

		/**
		 * @return DT_Shortcode_AlbumsMasonry
		 */
		...
		/**
		 * DT_Shortcode_AlbumsMasonry constructor.
		 */
		public function __construct() {
			$this->sc_name = 'dt_albums_masonry';
			$this->unique_class_base = 'albums-masonry-shortcode-id';
		...
		protected function do_shortcode( $atts, $content = '' ) {
			...
			echo '<div ' . $this->container_class( 'albums-shortcode' ) . presscore_masonry_container_data_atts( $data_atts ) . '>';
			...
			DT_Albums_Shortcode_HTML::display_posts_filter( $terms, $filter_class );

			/**
			 * Albums posts have a custom lazy loading classes.
			 * @see DT_Albums_Shortcode_HTML::get_post_image
			 */
    ...
    // 			}
				...
				presscore_get_template_part( 'mod_albums_shortcodes', 'albums-masonry/tpl-layout', $this->get_att( 'layout' ), array(
					//'post_media'          => $post_media,
					'details_btn'         => DT_Albums_Shortcode_HTML::get_details_btn( $details_btn_style, $details_btn_text, $follow_link, $details_btn_class ),
		...
		protected function container_class( $class = array() ) {
			...
			if ( function_exists( 'vc_shortcode_custom_css_class' ) ) {
				$class[] = vc_shortcode_custom_css_class( $this->atts['css_dt_albums'], ' ' );
			};
		...
		protected function get_less_file_name() {
			if ( the7pt_is_theme_version_smaller_or_equal_to( '6.7.0' ) ) {
				return get_template_directory() . '/css/dynamic-less/shortcodes/dt-albums-masonry.less';
			}

			return The7PT()->plugin_path() . 'assets/css/shortcodes/dt-albums-masonry.less';
		}
		...
		protected function get_less_vars() {
			...
			$less_vars->add_keyword( 'unique-shortcode-class-name', 'albums-shortcode.' . $this->get_unique_class(), '~"%s"' );
		...
		protected function get_posts_filter_terms( $query ) {
			...
			if ( 'standard' !== $this->get_att( 'loading_mode' ) ) {
				...
				return wp_get_object_terms( $post_ids, 'dt_gallery_category', array( 'fields' => 'all_with_object_id' ) );
			}
	...
	DT_Shortcode_AlbumsMasonry::get_instance()->add_shortcode();
