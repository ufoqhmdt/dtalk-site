	...
	class AIO_Just_Icon
	{
		...
		function just_icon_shortcode($atts)
		{
			...
			if($icon_link !== ''){
				...
				$link_prefix .= '<a class="aio-tooltip '.esc_attr($uniqid).'" href = "'. esc_url($url) .'" '.$target.' '. $rel .' '. $link_title .' data-toggle="tooltip" data-placement="'.esc_attr($tooltip_disp).'" title="'.esc_attr($tooltip_text).'">';
				$link_sufix .= '</a>';
			} else {
				if($tooltip_disp !== ""){
