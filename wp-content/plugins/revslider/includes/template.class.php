...
class RevSliderTemplate {
    ...
    public function __construct() {
        if (defined('REVSLIDER_THEME_ACTIVATED_URL')){
            $this->templates_url = REVSLIDER_THEME_ACTIVATED_URL . 'bundled-content/';
            $this->templates_list = 'revslider/templates/get-list.php';
            $this->templates_download = 'revslider/templates/download.php';
            $this->templates_server_path = 'revslider/templates/images/';
        }
    }
			...
			<ul class="ttm_requirements">
				...
				if(isset($template['plugin_require']) && !empty($template['plugin_require'])){
					foreach($template['plugin_require'] as $pk => $pr){
						...
						if(isset($pr['url'])) echo '<a href="'.admin_url('admin.php?page=rev_addon').'">';
						echo $pr['name'];
